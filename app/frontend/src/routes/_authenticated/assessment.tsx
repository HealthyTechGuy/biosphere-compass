...
export const Route = createFileRoute("/_authenticated/assessment")({
  head: () => ({ meta: [{ title: "Business assessment — Biosphere Beacons" }] }),
  component: AssessmentPage,
});

function AssessmentPage() {
  if (isDemoMode()) return <DemoAssessment />;
  return <SupabaseAssessment />;
}
      ...
      <div className="space-y-8">
        <section className="card-soft p-6" aria-labelledby="assessment-heading">
          <h2 id="assessment-heading" className="font-display text-xl font-semibold">
            Biosphere Beacons assessment
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Pick a sector, work through the 21 impact criteria, and get an instant sector-weighted
            0–100 Compass score with a priority-action list.
          </p>
          <div className="mt-6">
            <BiosphereAssessment />
          </div>
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-semibold">Business assessment</h1>
        ...
        {adminCheck?.admin && (
          <Link
            to="/admin"
            className="rounded-xl bg-secondary px-4 py-2 text-sm font-semibold text-secondary-foreground hover:bg-accent"
          >
            Admin dashboard →
          </Link>
        )}
...
function BusinessForm({
  ...
  const mutation = useMutation({
    mutationFn: () => saveFn({ data: { ...form, category_id: form.category_id || null } }),
    onSuccess: onSaved,
  });
    ...
    >
      <div className="sm:col-span-2">
        <h2 className="font-display text-xl font-semibold">
          {existing ? "Business details" : "Create your business profile"}
        </h2>
      </div>
      <div>
      ...
      <div>
        <label htmlFor="b-category" className="text-sm font-medium">
          Category
        </label>
        <select
          id="b-category"
      ...
      <div className="sm:col-span-2">
        <label htmlFor="b-tagline" className="text-sm font-medium">
          Tagline
        </label>
        <input
          id="b-tagline"
      ...
      <div className="sm:col-span-2">
        <label htmlFor="b-desc" className="text-sm font-medium">
          Description
        </label>
        <textarea
          id="b-desc"
      ...
      <div>
        <label htmlFor="b-parish" className="text-sm font-medium">
          Town / parish
        </label>
        <input
          id="b-parish"
      ...
      <div>
        <label htmlFor="b-website" className="text-sm font-medium">
          Website
        </label>
        <input
          id="b-website"
      ...
      <div className="flex items-end sm:col-span-2">
        <button
          ...
          className="btn-hero rounded-xl px-6 py-3 text-sm font-bold disabled:opacity-60"
        >
          {mutation.isPending ? "Saving…" : existing ? "Save changes" : "Create profile"}
        </button>
        {mutation.isError && (
          <p role="alert" className="ml-4 text-sm text-destructive">
