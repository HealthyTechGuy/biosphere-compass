...
// Client-side demo credentials — no backend, no Supabase call.
      ...
      if (mode === "signup") {
        const { error } = await supabase.auth.signUp({
          ...
          options: {
            ...
            data: { display_name: businessName || email.split("@")[0] },
          },
        });
    ...
    <div className="flex min-h-[70vh] items-center justify-center px-4 py-16">
      <div className="card-soft w-full max-w-md p-8">
        ...
        <button
          ...
          className="mt-6 flex w-full items-center justify-center gap-3 rounded-xl border border-border bg-card py-3 text-sm font-semibold transition-colors hover:bg-accent"
        >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="btn-hero w-full rounded-xl py-3 text-sm font-bold disabled:opacity-60"
          >
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          {mode === "signin" ? "New to the directory?" : "Already have an account?"}{" "}
          <button
        ...
        {mode === "signin" && (
          <p className="mt-4 rounded-xl bg-secondary/60 px-3 py-2 text-center text-xs text-muted-foreground">
            Demo: sign in with{" "}
            <code className="rounded bg-background px-1 py-0.5 font-mono">test@test.com</code>{" "}
            / <code className="rounded bg-background px-1 py-0.5 font-mono">test</code>
          </p>
        )}
      </div>
    </div>
